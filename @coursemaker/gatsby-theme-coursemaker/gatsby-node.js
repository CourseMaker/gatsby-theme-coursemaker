			...
			fields: {
				...
				author: {
					...
					resolve: (source, args, context) => {
						...
						for (
							let step = 0;
							step < source.author.length;
							step++
						) {
							let node = getNodesByType(`AuthorsYaml`).find(
...
exports.createPages = async ({ actions, graphql, reporter }, themeOptions) => {
	...
	if (useStrapi === "true") {
		...
		try {
			...
			dataSources.cms.courses = cmsData.data.cms.siteBuild.school.courses.map(
				normalize.normalizeCourses(
					cmsData.data.cms.siteBuild.school.school_prices,
				),
			);
	...
	try {
		...
		if (dataSources.local.courses.length) {
			dataSources.local.school.landing_page.image =
				dataSources.local.courses[0]?.landing_page?.image;
		}
	} catch (error) {
	...
	createPage({
		...
		component: require.resolve(
			"./src/templates/list-courses-page-template.js",
		),
		context: {
			school: liveSchool,
			courses: allCourses,
